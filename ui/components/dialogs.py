from PySide6.QtWidgets import (QDialog, QFrame, QVBoxLayout, QHBoxLayout, 
                               QLabel, QLineEdit, QPushButton, QListWidget, QListWidgetItem, QWidget, QStackedWidget, QScrollArea, QMessageBox, QComboBox)
...
# --- CustomDialog ve AddFriendDialog Aynı ---
...
class SettingsDialog(QDialog):
    # YENİ: servers_list parametresi eklendi
    def __init__(self, parent, is_dark_mode, lang_dict, current_name, current_email, servers_list):
        ...
        self.servers_list = servers_list # Tüm sunucuların listesi
    ...
    def setup_ui(self):
        ...
        btn_upload_avatar.clicked.connect(lambda: QMessageBox.information(self, "Bilgi", "Fotoğraf yükleme yakında eklenecektir.")); avatar_layout.addWidget(self.set_avatar); avatar_layout.addSpacing(15); avatar_layout.addWidget(btn_upload_avatar); avatar_layout.addStretch(); p_layout.addLayout(avatar_layout)
        ...
        self.btn_upgrade.clicked.connect(lambda: webbrowser.open("https://sizin-odeme-linkiniz.com")); acc_box_layout.addWidget(lbl_acc_box_title); acc_box_layout.addWidget(lbl_acc_type); acc_box_layout.addWidget(lbl_acc_limit); acc_box_layout.addSpacing(10); acc_box_layout.addWidget(self.btn_upgrade); p_layout.addWidget(self.acc_box)
        ...
        btn_save_p.clicked.connect(lambda: QMessageBox.information(self, "Bilgi", "Şifre güncellendi!")); p_layout.addSpacing(20); p_layout.addWidget(btn_save_p)
        self.scroll_area.setWidget(self.page_personal)

        # ==========================================
        # 2. SUNUCU AYARLARI (DROPDOWN VE CRUD)
        # ==========================================
        ...
        # --- YENİ: AÇILIR LİSTE (COMBOBOX) İLE SUNUCU SEÇİMİ ---
        lbl_select = QLabel(self.lang['srv_select'])
        lbl_select.setObjectName("bold_label")
        s_layout.addWidget(lbl_select)
        
        self.cb_managed_servers = QComboBox()
        self.cb_managed_servers.setMinimumHeight(40)
        self.cb_managed_servers.setCursor(Qt.PointingHandCursor)
        
        # Yalnızca admin olunan sunucuları filtrele (Eğer API dönüyorsa. Dönmüyorsa hepsini ekleriz)
        self.managed_servers = []
        for s in self.servers_list:
            # Şu an API'nizin 'role' döndürüp döndürmediğini bilmiyoruz, 
            # şimdilik listesindeki her sunucuyu yönetebilir varsayıyoruz. 
            # İleride API'ye "role" eklerseniz burayı: if s.get('role') == 'admin': yapın.
            self.managed_servers.append(s)
        ...
        if not self.managed_servers:
            ...
            for s in self.managed_servers:
                self.cb_managed_servers.addItem(f"🏢 {s.get('name', 'Sunucu')}", s.get('id'))
        ...
        # --- KANAL LİSTESİ VE BUTONLAR ---
        lbl_channels = QLabel(self.lang['srv_set_channels'])
        lbl_channels.setObjectName("bold_label")
        s_layout.addWidget(lbl_channels)
        
        self.settings_channel_list = QListWidget()
        self.settings_channel_list.setObjectName("channel_settings_list") 
        s_layout.addWidget(self.settings_channel_list)
        
        btn_crud_layout = QHBoxLayout()
        self.btn_add_ch = QPushButton(self.lang['btn_add_ch'])
        self.btn_add_ch.setObjectName("success_btn")
        self.btn_add_ch.setCursor(Qt.PointingHandCursor)
        
        self.btn_edit_ch = QPushButton(self.lang['btn_edit_ch'])
        self.btn_edit_ch.setObjectName("primary_btn")
        self.btn_edit_ch.setCursor(Qt.PointingHandCursor)
        
        self.btn_del_ch = QPushButton(self.lang['btn_del_ch'])
        self.btn_del_ch.setStyleSheet("background-color: #ed4245; color: white; border-radius: 6px; padding: 10px; font-weight: bold;")
        self.btn_del_ch.setCursor(Qt.PointingHandCursor)
        
        btn_crud_layout.addWidget(self.btn_add_ch)
        btn_crud_layout.addWidget(self.btn_edit_ch)
        btn_crud_layout.addWidget(self.btn_del_ch)
        ...
        # İlk açılışta ilk sunucunun kanallarını getir
        if self.managed_servers:
            self.on_managed_server_changed(0)

    # --- KANAL CRUD FONKSİYONLARI ---
    ...
    def on_channels_fetched(self, success, channels):
        ...
        if success and channels:
            for ch in channels:
                ...
                item.setData(Qt.UserRole + 1, ch_type)
    ...
    def on_add_channel_clicked(self):
        dialog = ChannelDialog(self, self.is_dark_mode, self.lang, mode="add")
    ...
    def on_edit_channel_clicked(self):
        ...
        if not selected or selected.flags() == Qt.NoItemFlags:
            return QMessageBox.warning(self, "Uyarı", "Lütfen düzenlemek için bir kanal seçin.")
            
        ch_id = selected.data(Qt.UserRole)
        ch_type = selected.data(Qt.UserRole + 1)
        ch_name = selected.text().split(" ", 1)[1] 
        
        dialog = ChannelDialog(self, self.is_dark_mode, self.lang, mode="edit", channel_name=ch_name, channel_type=ch_type)
        if dialog.exec() == QDialog.Accepted:
    ...
    def on_delete_channel_clicked(self):
        selected = self.settings_channel_list.currentItem()
        if not selected or selected.flags() == Qt.NoItemFlags:
            return QMessageBox.warning(self, "Uyarı", "Lütfen silmek için bir kanal seçin.")
            
        reply = QMessageBox.question(self, "Onay", f"'{selected.text()}' kanalını silmek istediğinize emin misiniz?\nBu işlem geri alınamaz.", QMessageBox.Yes | QMessageBox.No)
